...
correctDosages <- function(chk,
  ...
  seginfo <- segtypeInfoSummary(calcSegtypeInfo(ploidy))
  ...
  chk <- chk[ch.ord,]
  potShifts <- calcPotShifts(chk$bestfit, ploidy) #potential shift value for each marker
  ...
  shfpar <- (cbind(chk$parent1 + potShifts$shift, chk$parent2 + potShifts$shift))[mf,]
...
screen_for_duplicate_individuals <-
  function(dosage_matrix,
    ...
    if (length(gcl_nw) > 0) {
      ...
      write(paste("\n####",length(gcl_nw),"individuals removed:\n"), log.conn)
...
#' @param log Character string specifying the log filename to which standard output should be written. If NULL log is send to stdout.
#' @param min_bridges The minimum number of cross-parent bridges for a link to be considered.
...
#'                  parentname="P1")
#' @export
bridgeHomologues <- function(cluster_stack,
                             cluster_stack2 = NULL,
                             linkage_df,
                             linkage_df2 = NULL,
                             LOD_threshold = 10,
  ...
  if (is.null(cluster_stack2) & is.null(linkage_df2)) {
    ...
    if (length(gcl_linkage_df) > 0) {
      ...
      colnames(linkage_df_LGs) <- c("marker", "LG")
    } else {
  ...
  igraph::plot.igraph(
    ...
    vertex.color = "grey",
    main = paste(
      parentname,"SN+SN homologue links with LOD >",
      LOD_threshold
    )
  )
  ...
  if (automatic_clustering) {
    ...
    if (length(gcl) != LG_number) {
      warning(
        ...
        If you're not okay with that, try again with different LOD_cutoff or automatic_clustering = FALSE"
      )
    }
  } else {
...
#' @param bin_list A list of marker bins containing marker duplicates. Output of screen_for_duplicate_markers.
#'
#' @return
#' A maplist with added markers.
#' @export
add_dup_markers <- function(maplist, bin_list){
  ...
  for(lg in names(maplist)){
    ...
    maplist_out[[lg]] <- map[order(map$position),]
  }
  return(maplist_out)
}
